...
import { createComponent, unescapeHTML, render as serverRender } from 'astro/runtime/server/index.js';
 ...
 */
export function getPostSummary(slug: string | undefined): string | null {
  if (!slug) return null;
...
/**
 * Derive a content slug from a file path.
 * Mirrors Astro's getContentEntryIdAndSlug logic for store-path entries.
 * @param filePath e.g. "src/content/blog/en/post-name.md"
 * @returns slug portion, e.g. "en/post-name"
 */
...
export async function getSortedPosts(locale?: string): Promise<CollectionEntry<'blog'>[]> {
  return memoize('sortedPosts', locale ?? '__all__', async () => {
    ...
    // Patch entries that lack render()/slug when the content layer lazy-import map
    // is empty (common for markdown blog posts because addModuleImport is only
    // called for deferredRender entries). Builds a Content component from the
    // pre-rendered HTML that the content store already computed.
    for (const post of posts) {
      if (typeof (post as any).render !== 'function') {
        (post as any).render = async () => {
          const contentHtml = (post as any).rendered?.html;
          if (!contentHtml) {
            return {
              Content: createComponent(() => serverRender``),
              headings: [],
              remarkPluginFrontmatter: {}
            };
          }
          const Content = createComponent(() => serverRender`${unescapeHTML(contentHtml)}`);
          return {
            Content,
            headings: (post as any).rendered?.metadata?.headings ?? [],
            remarkPluginFrontmatter: (post as any).rendered?.metadata?.frontmatter ?? {}
          };
        };
      }
      if (!(post as any).slug && (post as any).filePath) {
        (post as any).slug = computeSlugFromPath((post as any).filePath);
      }
    }
...
export async function getPostById(id: string, locale?: string): Promise<CollectionEntry<'blog'> | undefined> {
  const map = await memoize('postByIdMap', locale ?? '__all__', async () => {
    ...
    const result = new Map(posts.map((p) => [p.id, p]));
    return result;
  });
  return map.get(id);
}
