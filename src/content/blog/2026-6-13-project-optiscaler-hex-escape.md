...
| `\0`–`\7` （八进制） | **最多 3 位**：消费最多 3 个八进制字符 | `\78` = `\07` 后跟 `'8'` |
| `\u` （通用字符名） | **固定 4 位**：严格消费 4 个十六进制字符 | `\u00E9` = é |
| `\U` （通用字符名） | **固定 8 位**：严格消费 8 个十六进制字符 | `\U0001F600` = 😀 |
