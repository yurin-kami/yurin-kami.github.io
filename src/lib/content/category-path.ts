...
export function buildCategoryPath(categoryNames: string | string[]): string {
  ...
  for (const name of names) {
    ...
    if (!mapped) {
      console.warn(`[buildCategoryPath] Category "${name}" not found in categoryMap. Check config/site.yaml.`);
      slugs.push(name.toLowerCase().replace(/\s+/g, '-'));
    } else {
      slugs.push(encodeSlug(mapped));
    }
  }
